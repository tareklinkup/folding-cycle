...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases()
    {
        ...
        if (isset($data->lotNo) && $data->lotNo != '') {
            $clauses .= " and pm.PurchaseMaster_LotNo = '$data->lotNo'";
        }
        ...
        if (isset($data->purchaseId) && $data->purchaseId != null) {
            ...
            $res['purchaseDetails'] = $this->db->query("
                ...
                join tbl_brand bc on bc.brand_SiNo = p.brand
                join tbl_unit u on u.Unit_SlNo = p.Unit_ID
                where pd.Status <> 'd'
                and pd.PurchaseMaster_IDNo = '$data->purchaseId'
            ")->result();
        }
    ...
    public function purchaseEdit($purchaseId)
    {
        ...
        $data['lot_no'] = $this->db->query("select PurchaseMaster_LotNo from tbl_purchasemaster where PurchaseMaster_SlNo = ?", $purchaseId)->row()->PurchaseMaster_LotNo;
        $data['content'] = $this->load->view('Administrator/purchase/purchase_order', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                $purchaseDetails = array(
                    ...
                    'PurchaseDetails_SaleRate'      => $product->salesRate,
    ...
    public function approvePurchase()
    {
        ...
        try {
            ...
            foreach ($purchaseDetails as $key => $product) {
                ...
                // $getProduct = $this->db->query("select * from tbl_product where Product_SlNo = ? and Product_branchid = ?", [$product->Product_IDNo, $this->session->userdata('BRANCHid')])->row();

                $this->db->query("UPDATE tbl_product set 
                    Product_Purchase_Rate = (((Product_Purchase_Rate * ?) + ?) / ?), 
                    Product_SellingPrice = ? 
                    where Product_SlNo = ?
                ", [
                    ...
                    $product->PurchaseDetails_TotalAmount,
                    ($previousStock + $product->PurchaseDetails_TotalQuantity),
                    $product->PurchaseDetails_SaleRate,
                    $product->Product_IDNo
                ]);
            }
    ...
    public function  deletePurchase()
    {
        ...
        try {
            ...
            $purchaseDetails = $this->db->select('Product_IDNo,PurchaseDetails_TotalQuantity,PurchaseDetails_TotalAmount,Status')->where('PurchaseMaster_IDNo', $data->purchaseId)->get('tbl_purchasedetails')->result();
