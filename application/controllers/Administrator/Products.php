    ...
    public function getTotalStock()
    {
        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(fsd.Free_quantity), 0) 
                    from tbl_saledetails fsd
                    join tbl_salesmaster sm on sm.SaleMaster_SlNo = fsd.SaleMaster_IDNo
                    where fsd.FreeProduct_Id = p.Product_SlNo
                    and fsd.SaleDetails_BranchId  = '$branchId'
                    and fsd.Status = 'a'
                    " . (isset($data->date) && $data->date != null ? " and sm.SaleMaster_SaleDate <= '$data->date'" : "") . "
                ) as free_quantity,
                ...
                (select (purchased_quantity + sales_returned_quantity + transferred_to_quantity) - (sold_quantity + free_quantity + purchase_returned_quantity + damaged_quantity + transferred_from_quantity)) as current_quantity,
    ...
    public function getProductLedger()
    {
        ...
        $result = $this->db->query("
            ...
            select 
                ...
                sd.SaleDetails_SlNo as id,
                sm.SaleMaster_SaleDate as date,
                concat('Sale Free Qty- ', sm.SaleMaster_InvoiceNo, ' - ', c.Customer_Name) as description,
                0 as rate,
                0 as in_quantity,
                0 as out_quantity,
                sd.Free_quantity as free_quantity
            from tbl_saledetails sd
            join tbl_salesmaster sm on sm.SaleMaster_SlNo = sd.SaleMaster_IDNo
            join tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
            where sd.Status = 'a'
            and sd.FreeProduct_Id = " . $data->productId . "
            and sd.SaleDetails_BranchId = " . $this->brunch . "
        ...
        $ledger = array_map(function ($key, $row) use ($result) {
            $row->stock = $key == 0 ? $row->in_quantity - ($row->out_quantity + $row->free_quantity) : ($result[$key - 1]->stock + ($row->in_quantity - ($row->out_quantity + $row->free_quantity)));
            return $row;
