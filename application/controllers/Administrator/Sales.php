...
class Sales extends CI_Controller
{
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                $saleDetails = array(
                    ...
                    'SaleDetails_LotNo'         => $cartProduct->lotNo,
                    ...
                    'FreeProduct_Id'            => $cartProduct->freeProductId,
                    'Free_quantity'             => $cartProduct->freeQuantity,
                    'FreeProduct_LotNo'         => $cartProduct->freeProductLotNo,
                    'FreeProduct_Purchase_Rate' => $cartProduct->FreeProduct_Purchase_Rate,
                ...
                //update Free product stock
                $this->db->query("
                    update tbl_currentinventory 
                    set sales_free_quantity = sales_free_quantity + ?
                    where product_id = ?
                    and branch_id = ?
                ", [$cartProduct->freeQuantity, $cartProduct->freeProductId, $this->session->userdata('BRANCHid')]);
    ...
    public function getSalesRecord()
    {
        ...
        $clausesLot = "";
        if (isset($data->lotNo) && $data->lotNo != '') {
            $clausesLot .= " and sd.SaleDetails_LotNo = '$data->lotNo'";
        }
        ...
        if (isset($data->salesId) && $data->salesId != 0 && $data->salesId != '') {
            ...
            $saleDetails = $this->db->query("
                select 
                    ...
                    p.Product_Name,
                    pc.ProductCategory_Name,
                    bc.brand_name,p1.Product_Name as free_product_name, p1.brand as free_product_brand,
                u.Unit_Name
                from tbl_saledetails sd
                left join tbl_product p1 on p1.Product_SlNo = sd.FreeProduct_Id
                    left join tbl_product p on p.Product_SlNo = sd.Product_IDNo
                    left join tbl_productcategory pc on pc.ProductCategory_SlNo = p.ProductCategory_ID
                    left join tbl_brand bc on bc.brand_SiNo = p.brand
        ...
        try {
            ...
            foreach ($currentSaleDetails as $product) {
                $this->db->query("
                    ...
                    where product_id = ?
                    and branch_id = ?
                ", [$product->Free_quantity, $product->FreeProduct_Id, $this->session->userdata('BRANCHid')]);
...
<?php
    ...
    public function getProfitLoss()
    {
        ...
        if ($data->product != null && $data->product != '') {
            $productClause = " and sd.Product_IDNo = '$data->product'";
        }

        $freeProductClause = "";
        if ($data->product != null && $data->product != '') {
            $freeProductClause = " and sd.FreeProduct_Id = '$data->product'";
        }

        $LotClause = "";
        if ($data->lotNo != null && $data->lotNo != '') {
            $LotClause = " and sd.SaleDetails_LotNo = '$data->lotNo'";
        }

        $FreeLotClause = "";
        if ($data->lotNo != null && $data->lotNo != '') {
            $FreeLotClause = " and sd.FreeProduct_LotNo = '$data->lotNo'";
        }
        ...
        foreach ($sales as $sale) {
            $sale->saleDetails = $this->db->query("SELECT
                    ...
                    sd.SaleDetails_LotNo as SaleDetails_LotNo,
                    sd.SaleDetails_TotalQuantity as SaleDetails_TotalQuantity,
                    sd.Purchase_Rate as Purchase_Rate,
                    sd.SaleDetails_Rate as SaleDetails_Rate,
                    (sd.Purchase_Rate * sd.SaleDetails_TotalQuantity) as purchased_amount,
                    sd.SaleDetails_TotalAmount as SaleDetails_TotalAmount,
                    (select sd.SaleDetails_TotalAmount - purchased_amount) as profit_loss

                from tbl_saledetails sd 
                join tbl_product p on p.Product_SlNo = sd.Product_IDNo
                where sd.SaleMaster_IDNo = '$sale->SaleMaster_SlNo'
                $productClause $LotClause
                ...
                SELECT
                    p.Product_Code as Product_Code,
                    p.Product_Name as Product_Name,
                    sd.FreeProduct_LotNo as SaleDetails_LotNo,
                    sd.Free_quantity as SaleDetails_TotalQuantity,
                    sd.FreeProduct_Purchase_Rate as Purchase_Rate,
                    0 as SaleDetails_Rate,
                    (sd.FreeProduct_Purchase_Rate * sd.Free_quantity) as purchased_amount,
                    0 as SaleDetails_TotalAmount,
                    (select 0 - purchased_amount) as profit_loss

                from tbl_saledetails sd 
                join tbl_product p on p.Product_SlNo = sd.FreeProduct_Id
                where sd.SaleMaster_IDNo = '$sale->SaleMaster_SlNo'
                $freeProductClause $FreeLotClause
    ...
    function getProductLots()
    {
        ...
        (SELECT pd.PurchaseDetails_TotalQuantity - (sale_qty + sale_free_qty)) as remaining_qty
    ...
    function getSaleLots()
    {
        ...
        $result = $this->db->query("SELECT SaleDetails_LotNo
        FROM tbl_saledetails
        WHERE Status = 'a'
        and SaleDetails_BranchId = ?
        GROUP by SaleDetails_LotNo
        ", $this->session->userdata("BRANCHid"))->result();
