			...
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				<div class="form-group">
					...
					<select class="form-control" v-model="searchType" @change="onChangeSearchType">
						...
						<option value="freequantity">By Free Quantity</option>
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'freequantity' && products.length > 0 ? '' : 'none'}">
					<label>Product</label>
					<v-select v-bind:options="products" v-model="selectedfreeProduct" label="display_text" @input="sales = []"></v-select>
				</div>
				...
				<div class="form-group" v-bind:style="{display: searchTypesForRecord.includes(searchType) && searchType != 'lot' ? '' : 'none'}">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td>{{ sale.saleDetails[0].SaleDetails_LotNo }}</td>
				...
				<template v-if="searchTypesForDetails.includes(searchType)" style="display:none;" v-bind:style="{display: searchTypesForDetails.includes(searchType) ? '' : 'none'}">
					<table class="record-table" v-if="selectedProduct != null || selectedfreeProduct != null">
						<thead>
							<tr>
								<th>Invoice No.</th>
								<th>Date</th>
								...
								<th>Product Name</th>
								<th>Sales Rate</th>
								<th v-if="searchType == 'quantity'">Quantity</th>
								<th v-else-if="searchType == 'freequantity'">Free Quantity</th>
							</tr>
						</thead>
						<tbody>
							<tr v-for="sale in sales">
								...
								<td v-if="searchType == 'quantity'" style="text-align:right;">{{ sale.SaleDetails_TotalQuantity }}</td>
								<td v-else style="text-align:right;">{{ sale.Free_quantity }}</td>
							</tr>
						</tbody>
						<tfoot>
							<tr style="font-weight:bold;">
								<td colspan="5" style="text-align:right;">Total Quantity</td>
								<td v-if="searchType == 'quantity'" style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.SaleDetails_TotalQuantity)}, 0) }}</td>
								<td v-else style="text-align:right;" style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.Free_quantity)}, 0) }}</td>
							</tr>
						</tfoot>
					</table>
					...
					<table class="record-table" v-if="selectedProduct == null && selectedfreeProduct == null">
						<thead>
							<tr>
								...
								<th v-if="searchType == 'quantity' || searchType == 'category'">Quantity</th>
								<th v-else-if="searchType == 'freequantity'">Free Quantity</th>
							</tr>
						</thead>
						<tbody>
	...
	new Vue({
		...
		data() {
			return {
				...
				sales: [],
				searchTypesForRecord: ['', 'user', 'customer', 'employee', 'lot'],
				searchTypesForDetails: ['freequantity', 'quantity', 'category']
			}
		},
		methods: {
			...
			onChangeSearchType() {
				...
				if (this.searchType == 'freequantity') {
					this.getProducts();
			...
			getSalesRecord() {
				let filter = {
					...
					lotNo: this.selectedLot == null || this.selectedLot.SaleDetails_LotNo == '' ? '' : this.selectedLot.SaleDetails_LotNo,
					...
					.then(res => {
						if (this.recordType == 'with_details') {
							if (this.searchType == 'lot') {
								this.sales = res.data.filter((obj) => {
			...
			getSaleDetails() {
				let filter = {
					...
					productId: this.selectedfreeProduct == null || this.selectedfreeProduct.Product_SlNo == '' ? '' : this.selectedfreeProduct.Product_SlNo,
					dateFrom: this.dateFrom,
					dateTo: this.dateTo
				}
				...
				axios.post('/get_saledetails', filter)
					.then(res => {
						...
						if (this.selectedProduct == null && this.selectedfreeProduct == null) {
							if (this.searchType == 'quantity' || this.searchType == 'category') {
								sales = _.chain(sales)
									...
									.value();
							} else if (this.searchType == 'freequantity') {
								sales = _.chain(sales)
									.groupBy('ProductCategory_ID')
									.map(sale => {
										return {
											...
											products: _.chain(sale)
												...
												.map(product => {
													return {
														product_code: product[0].Product_Code,
														product_name: product[0].Product_Name,
														quantity: _.sumBy(product, item => Number(item.Free_quantity))
													}
												})
												.value()
										}
			...
			async print() {
				...
				let productfreeText = '';
				if (this.selectedfreeProduct != null && this.selectedfreeProduct.Product_SlNo != '' && this.searchType == 'freequantity') {
					productfreeText = `<strong>Product: </strong> ${this.selectedfreeProduct.Product_Name}`;
				}
				let productText = '';
