		...
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'lot' ? '' : 'none'}">
					<label>Lot No</label>
					<v-select v-bind:options="lots" v-model="selectedLot" label="PurchaseMaster_LotNo"></v-select>
				</div>
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
					...
					<tbody>
						<template v-for="purchase in purchases">
							<tr>
								...
								<td>{{ purchase.PurchaseMaster_LotNo }}</td>
								...
								<td style="text-align:center;">
									...
									<?php if ($this->session->userdata('accountType') != 'u') { ?>
										<a v-if="purchase.status == 'p'" href="" title="Approve" @click.prevent="approveItem(purchase.PurchaseMaster_SlNo)"><i style="font-size:18px" class="fa fa-check-square"></i></a>
				...
				<table class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
					...
					<tbody>
						<template v-for="purchase in purchases">
							<tr :style="{background: purchase.status == 'p' ? 'rgb(255 251 220)' : ''}">
								<td>{{ purchase.PurchaseMaster_InvoiceNo }}</td>
								<td>{{ purchase.PurchaseMaster_LotNo }}</td>
								<td>{{ purchase.PurchaseMaster_OrderDate }}</td>
								<td>{{ purchase.Supplier_Name }}</td>
	...
	new Vue({
		...
		data() {
			return {
				...
				purchases: [],
				searchTypesForRecord: ['', 'user', 'supplier', 'lot'],
		...
		methods: {
			...
			getLotNo() {
				axios.get('/get_purchase_lots').then(res => {
					this.lots = res.data;
				})
			},
			getSuppliers() {
			...
			getPurchaseRecord() {
				let filter = {
					...
					lotNo: this.selectedLot == null ? '' : this.selectedLot.PurchaseMaster_LotNo,
			...
			approveItem(purchaseId) {
				let deleteConf = confirm('Are you sure to approve item?');
				if (deleteConf == false) {
					return;
				}
				axios.post('/approve_purchase', {
						purchaseId: purchaseId
					})
					.then(res => {
						let r = res.data;
