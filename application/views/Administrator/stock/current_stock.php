...
<div id="stock">
    <div class="row">
        <div class="col-xs-12 col-md-12 col-lg-12" style="border-bottom:1px #ccc solid;margin-bottom:5px;">
            ...
            <div class="form-group" style="margin-top:10px;" v-if="selectedSearchType.value == 'lot'">
                <div class="col-sm-2" style="margin-left:15px;">
                    <v-select v-bind:options="lots" v-model="selectedLot" label="display_name"></v-select>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="table-responsive" id="stockContent">
                ...
                <table class="table table-bordered" v-if="searchType == 'lot'" style="display:none"
                    v-bind:style="{display: searchType == 'lot' ? '' : 'none'}">
                    <thead>
                        <tr>
                            <th>Product Id</th>
                            <th>Product Name</th>
                    ...
                    <tbody>
                        <tr v-for="product in stock">
                            ...
                            <td>{{ product.ProductCategory_Name }}</td>
                            <td>{{ product.current_quantity }} {{ product.Unit_Name }}</td>
                            <td>{{ product.PurchaseDetails_Rate | decimal }}</td>
                            <td>{{ product.stock_value | decimal }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                ...
                <table class="table table-bordered" v-if="searchType != 'current' && searchType != null"
                    style="display:none;"
                    v-bind:style="{display: searchType != 'current' && searchType != 'lot' && searchType != null ? '' : 'none'}">
                    <thead>
                        <tr>
                            <th>Product Id</th>
                            <th>Product Name</th>
...
new Vue({
    ...
    methods: {
        getStock() {
            ...
            if (this.searchType == 'lot' && this.selectedLot == null) {
                alert('Select a Lot Number');
                return;
            } else if (this.searchType == 'lot' && this.selectedLot != null) {
                parameters.lot_no = this.selectedLot.display_name;
                this.selectionText = "lot: " + this.selectedLot.display_name;
            }
        ...
        onChangeSearchType() {
            if (this.selectedSearchType.value == 'category' && this.categories.length == 0) {
                ...
                this.getProducts();
            } else if (this.selectedSearchType.value == 'lot' && this.lots.length == 0) {
                this.getLots();
            }
        },
        getCategories() {
