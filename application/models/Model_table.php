...
class Model_Table extends CI_Model
{
    ...
    public function generatePurchaseLotNo()
    {
        $lotNo = "LOT0001";
        $purchasesLot = $this->db->query("SELECT PurchaseMaster_LotNo FROM tbl_purchasemaster ORDER by PurchaseMaster_SlNo DESC LIMIT 1");

        if ($purchasesLot->num_rows() != 0) {
            $lastNo = substr($purchasesLot->row()->PurchaseMaster_LotNo, -4) + 1;
            if (strlen($lastNo) == 1) {
                $lotNo = "LOT000" . $lastNo;
    ...
    public function currentStock($clauses = '')
    {
        $stock = $this->db->query("
            select * from(
                select
                    ci.*,
                    (select (ci.purchase_quantity + ci.sales_return_quantity + ci.transfer_to_quantity) - (ci.sales_quantity + ci.sales_free_quantity + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity)) as current_quantity,
                    p.Product_Name,
                    p.Product_Code,
    ...
    public function productStock($productId)
    {
        ...
        if ($stockCount != 0) {
            ...
            $stock = ($stockRow->purchase_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity)
                - ($stockRow->sales_quantity + $stockRow->sales_free_quantity + $stockRow->purchase_return_quantity + $stockRow->damage_quantity + $stockRow->transfer_from_quantity);
        }
